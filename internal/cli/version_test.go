...
func TestDefaultScaffoldCaisVersion_is081(t *testing.T) {
	if defaultScaffoldCaisVersion != "0.8.1" {
		t.Errorf("defaultScaffoldCaisVersion = %q, want 0.8.1 so amarra-cais new pins the tagged release", defaultScaffoldCaisVersion)
	}
}

func TestREADME_goInstallPinsV081(t *testing.T) {
	...
	if !strings.Contains(text, "amarra-cais@v0.8.1") {
		t.Error("README go install should pin @v0.8.1")
	}
	if strings.Contains(text, "amarra-cais@v0.8.0") {
		t.Error("README still pins @v0.8.0")
	}
}
