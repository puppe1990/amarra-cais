...
const tplAgentsMD = "# {{.AppName}} — AI Conventions\n\n" +
	...
	"Shipped hooks: `clipboard`, `dialog` (native `<dialog>` via `amarra-hook=\"dialog\"` + `data-amarra-dialog-open`/`-target`/`-close`; `<.modal>` renders the target), `dropdown` (row actions: `amarra-hook=\"dropdown\"` + `data-amarra-dropdown-button`/`-menu`), `nav` (re-sync active link after Drive morph: `amarra-hook=\"nav\"` + `data-amarra-nav-on`/`-off` on the container), `password`, `reveal` (client show/hide, no Drive round-trip), `theme` (`html.light` + `localStorage[\"amarra-theme\"]`, per-element via `data-amarra-theme-key` / `-class` / `-color` / `-on-label` / `-off-label`).\n" +
