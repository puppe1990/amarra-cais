...
const tplAgentsMD = "# {{.AppName}} — AI Conventions\n\n" +
	...
	"Shipped hooks: `clipboard`, `password`, `reveal` (client show/hide, no Drive round-trip), `theme` (`html.light` + `localStorage[\"amarra-theme\"]`, per-element via `data-amarra-theme-key` / `-class` / `-color` / `-on-label` / `-off-label`).\n" +
