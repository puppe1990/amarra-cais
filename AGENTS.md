...
<button type="button" amarra-hook="password" data-amarra-password-for="#password"
        data-amarra-label-show="Show password" data-amarra-label-hide="Hide password">Show</button>
...
Shipped `amarra-hook` builtins: `bulk` (select-all on the current page: container `amarra-hook="bulk"` with `input[data-amarra-bulk-all]` + row checkboxes `input[data-amarra-bulk-row]`; header shows `indeterminate` when some rows are selected; optional `[data-amarra-bulk-bar]` appears when count > 0 with `[data-amarra-bulk-count]` as the label; the action itself stays a Drive POST/DELETE of the selected ids), `clipboard`, `dialog` (native `<dialog>`: container `amarra-hook="dialog"` with `button[data-amarra-dialog-open]` + `dialog[data-amarra-dialog-target]`; `showModal()` gives Esc, focus trap and `::backdrop`; `aria-modal="true"` set on connect; focus returns to the opener on close; optional `button[data-amarra-dialog-close]`; `<.modal>` renders the dialog target), `dropdown` (row actions / overflow menus: container `amarra-hook="dropdown"` with `button[data-amarra-dropdown-button aria-expanded]` + `div[data-amarra-dropdown-menu hidden]`; click toggles, Esc and outside clicks close, menu-item clicks close after acting), `nav` (re-sync active link after Drive morph: set `amarra-hook="nav"` + `data-amarra-nav-on` / `data-amarra-nav-off` class lists on the nav container; matches `a.pathname === location.pathname`, ignores query, sets `aria-current="page"`, runs on connect / `amarra:morphed` / `popstate`), `password` (toggle `type` + `aria-pressed`; falls back to the closest sibling `input` when `data-amarra-password-for` is missing, swaps `aria-label` via `data-amarra-label-show`/`data-amarra-label-hide`, icons accept `data-amarra-password-icon` with `data-cais-password-icon` as legacy alias), `reveal` (show/hide a target when the control value matches, no Drive round-trip), `theme` (toggle `html.light`, persist `localStorage["amarra-theme"]`, optional `theme-color` meta). The theme hook is configurable per element (#30): `data-amarra-theme-key` (storage key, default `amarra-theme`), `data-amarra-theme-class` (default `light`), `data-amarra-theme-color` / `data-amarra-theme-color-off` (`theme-color` meta hex), `data-amarra-theme-on-label` / `data-amarra-theme-off-label` (swap button text; keeps `aria-pressed`). If you set a custom key, copy it into the FOUC snippet below so restore matches. Put this FOUC snippet in the layout `<head>` before CSS so a stored light theme does not flash dark:
...
Shipped kit (override in `web/templates/components/`): `form`, `input`, `password` (input + eye toggle wired to `amarra-hook="password"`, supports `name`/`label`/`value`/`error`/`autocomplete`/`required`), `select`, `textarea`, `checkbox`, `button`, `flash`, `nav`, `pagination`, `modal`, `locale-toggle`, `stat` (KPI card: `label`/`value` + optional `delta`/`hint`/`href`), `empty` (empty state: `title`, body slot, optional `href`/`action` link), `table` (rows in the `.Inner` slot; `cols` from page data — `Field`/`Label`/`Sortable` maps — plus `sort`/`dir` for server-side sort links (`?sort=&dir=`, toggles dir, `aria-sort`), optional `frame` so only the table morphs), `filters` (GET form: `action`, optional `frame`/`submit`/`clear` href, fields in the `.Inner` slot become query params — hidden inputs preserve `sort`/`page`). One slot: `.Inner`. Self-closing `<.flash />` is allowed. `<.form>` injects `csrf_token` from `.CSRFToken`. File uploads: `<.input type="file" accept="image/*" />` inside `<.form enctype="multipart/form-data">` (do not set `value` on file inputs).
