...
func FieldSelect(f SelectFieldData) template.HTML {
	...
	b.WriteString(`</label><select data-cais-select-search="true" class="w-full border border-slate-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-copper outline-none" id="`)
	b.WriteString(template.HTMLEscapeString(f.Name))
	b.WriteString(`" name="`)
	b.WriteString(template.HTMLEscapeString(f.Name))
	b.WriteString(`"`)
